...
public class ManagerDaoImpl implements ManagerDao{
	...
	public Long insert(Manager bean)  {
		   ...
		   Integer num=0;
	 	   conn = DbPub.getConn();
		   String sql="insert into manager (managerid,loginname,loginpass,nickname,isdisabled)values(?,?,?,?,?)";
	...
	public Long update(Manager bean) {
		 java.sql.Connection conn=null;
		   ...
		   Integer num=0;
	 	   conn = DbPub.getConn();
		   String sql="update manager set loginname =?, loginpass=? , nickname= ?, isdisabled=? where managerid = ?";
	...
	public List<Manager> list() {
		  java.sql.Connection conn=null;
		   ...
		   rs=DbPub.query(conn, sql);
		    List<Manager> bean=new ArrayList<>();
			try {
				while (rs.next()) {
				bean.add(toBean(rs));
				}
	...
	public Long count() {
		...
		String sql="select count(1) from manager where 1=1";
	...
	public Long countByName(String name) {
		...
		Long num=0l;
		List<Object> list=new ArrayList<>();
		String sql="select count(1) from manager where bookname=?";
		Object[] params = list.toArray();
		num=DbPub.queryScalearLong(conn, sql, params);
		DbPub.closeConnection(conn);
		return num;
	...
	public List<Manager> listByName(String name) {
		  List<Manager>bean1=new ArrayList<>();
		   java.sql.Connection conn=null;
		   ResultSet rs=null;
		   conn = DbPub.getConn();
		   String sql="select * from Manager where nickname like ? ";
		    List<Object> list=new ArrayList<Object>();
		    name="%"+name+"%";
			list.add(name);
			 Object[] params = list.toArray();
