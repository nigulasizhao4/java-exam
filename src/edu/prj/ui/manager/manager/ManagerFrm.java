	...
	void showListDate() {
		...
		java.util.List<Manager> list=null;
		 ...
		 if(name!=null&&!name.isEmpty()) {
			 list=managerService.listByName(name);
		 }else {
			list=managerService.list();
		}
		 
		 ATableModel tableModel = null;
		tableModel = new ATableModel<Manager>(list,5) {

			@Override
			public Object getPropValue(Manager bean, int columnIndex) {
				if(columnIndex==0) {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == dMenuItem) {
			...
			if(index!=-1) {
				...
				if(buttonValue==JOptionPane.YES_OPTION) {
					ManagerService managerService = new ManagerServiceImpl();
					Long result=managerService.delete(pk);
					if(result>0) {
        ...
        if (e.getSource() == uMenuItem) {
			...
			if(index!=-1) {
				...
				if(pk>0) {
        	    ManagerUpdateFrm managerUpdateFrm = new ManagerUpdateFrm();
        	    
        	   managerUpdateFrm.pk=pk;
        	   managerUpdateFrm.loadData();
        	   
        	   managerUpdateFrm.managerFrm=this;	
        	   managerUpdateFrm.setVisible(true);
		 ...
		 if (e.getSource() == btnInsert) {
			 ManagerInsertFrm managerInsertFrm = new ManagerInsertFrm();
			 managerInsertFrm.managerFrm=this;
			 
			 managerInsertFrm.setVisible(true);
			 this.setVisible(false);		 
	        }
